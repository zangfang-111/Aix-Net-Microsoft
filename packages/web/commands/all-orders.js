...
export const allOrdersCommand = async (ctx, broker) => {
  const user = ctx.message.from

  await broker.call('session.clearUserSession', { userId: user.id })

  const orders = await broker.call('mob.order.findOrders', { telegramId: user.id })
  let messages = []
  ...
  if (orders.length === 0) {
    ...
    messages.push(
      ...
      ...sortedOrder.map(order => ({ text: `${orderMessage(order)} ${order.status}` }))
    )
  }
