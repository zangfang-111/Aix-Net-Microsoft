...
async function quoteRequestCanceled (quoteRequest, broker) {
  ...
  // Get RFQs with quoteRequestId and update them with Canceled status
  ...
  rfqsList.map(async rfq => {
    ...
    broker.call('web.pushBatch', {
      messages: [{
        ...
        text: `/${rfq.rfqId} This ${rfq.volume} ${rfq.financialInstrument.label} was cancelled.`
      }]
    })
  })
}
